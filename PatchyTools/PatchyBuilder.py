...
class PSCN():
    ...
    def process_data(self, X, y=None):
        ...
        train = []
        for i in range(n):
            rfMaker = ReceptiveFieldMaker(X[i], w=self.w, k=self.k, s=self.s
...
class ReceptiveFieldMaker():
    ...
    def make_(self):
        ...
        for graph in f:
            ...
            sorted_idx = sorted(nx.get_node_attributes(frelabel, 'attr_name').items(), key=lambda x: x[0])
            if self.one_hot > 0:
                forcnn.append([utils.indices_to_one_hot(x[1], self.one_hot) for x in sorted_idx])
    ...
    def normalize_graph(self, subgraph, vertex):
        ...
        original_order_to_respect = nx.get_node_attributes(ranked_subgraph_by_labeling_procedure,
                                                           'labeling')  # à changer je pense
